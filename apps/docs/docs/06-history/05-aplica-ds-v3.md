...
Versão atual: **3.20.0** (2026-05-22)

- **3.20.0** — Override `background` em estado de interação + correções de acessibilidade + gate de testes: quarto campo no leaf de override (`background` substitui a surface de um estado diretamente — `txtOn` auto-computado WCAG AA, sem paleta necessária); fix de override txtOnly descartado silenciosamente em estados ghost com dilution-factor (via `pendingTxtOnlyOverride`); enforcement WCAG AA em todos os pontos de aplicação de override com warning nomeado `[override:accessibility]`; 171 testes novos em quatro suítes (`test:interaction-overrides`, `test:config-matrix`, `test:txtOn-strategies`, `test:decomposition-variants`); `build:themes` agora exige que `test:theme-engine` passe antes de avançar; `prerelease:check` agrega todos os gates de release; Seções 7–8 do `RULES.md` formalizadas (contrato de acessibilidade de overrides e retrocompatibilidade como requisito inegociável). 100% retrocompatível — `background` é opt-in e `pendingTxtOnlyOverride` só afeta overrides que antes eram silenciosamente descartados.
