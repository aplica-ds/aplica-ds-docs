...
| `theme-engine preview` | Comando CLI | "`npx theme-engine preview` — gera preview HTML estático em `dist/preview/`; flags: `--build` (rebuild antes), `--serve` (servidor local com live reload); dropdown **View** com modos Detailed / Summary; desde 3.17.0: layout de referência com nav hierárquica semântica, modo compacto e filtro de busca em tempo real — desde 3.9.0" |
...
| `theme-engine ai:init` | Comando CLI | "injeta arquivos de contexto do pacote publicado no workspace consumidor; desde 3.10.0, distribui `AI_CONTEXT.md` e `THEME_CONFIG_REFERENCE.md` diretamente do pacote, atualizados a cada upgrade; desde 3.15.x, também copia `DESIGN.md` estático; desde 3.16.1, também instala `aplica-knowledge-guide` em todas as ferramentas de IA" |
| `theme-engine ai:knowledge` | Comando CLI | "`npx theme-engine ai:knowledge` — instala skill conversacional `aplica-knowledge-guide` em `.claude/skills/`, `.cursor/rules/`, `.github/instructions/` e `docs/context/`; habilita o assistente a responder perguntas de config, explicar arquitetura e diagnosticar tokens lendo arquivos reais do workspace; aliases: `ai:guide`, `knowledge:init` — desde 3.16.1" |
| `theme-engine audit:accessibility` | Comando CLI | "`npx theme-engine audit:accessibility` — audita pares background/foreground de todos os brands contra critérios WCAG 2.1; calcula taxa de contraste e classifica AA, AAA ou FAIL; requer `dist/` gerado — desde 3.15.1" |
| `global.surfacePolarity` | Chave de config global | "em `themes.config.json` → `global.surfacePolarity: false` colapsa o workspace para positive-only: `data/surface/negative.json` é removido, `mode` e `brand` param de emitir branches de polaridade, e os outputs semânticos finais geram apenas arquivos `*-positive`; use em projetos sem dark-on-light ou inverse surfaces — desde 3.14.1" |
| `options.interaction.surfaces.ghost.ghostNormalTxtOnStrategy` | Chave de config | "controla como `txtOn` é calculado para o estado `ghost.normal` (background transparente): `'txt'` (padrão desde 3.15.1) usa cor interativa da paleta contra o canvas; `'surface'` executa `txtOnStrategy` do tema contra o canvas background (comportamento anterior); sobrescrevível por grupo via `options.interaction.groups.<group>.surfaces.ghost.ghostNormalTxtOnStrategy` — desde 3.15.1" |
| `options.txtOnStrategy (colorContrastDecompose)` | Chave de config | "quando `txtOnStrategy: 'brand-tint'`, `colorContrastDecompose` controla a direção do walk e o extremo de fallback: `'startDark'` (padrão) prioriza tons escuros / extremo = preto; `'startLight'` prioriza tons claros / extremo = branco; usado também por `overlap` — desde 3.18.0" |
| `options.txtOnStrategy (overlap)` | Chave de config | "quando `txtOnStrategy: 'brand-tint'`, `overlap: { level: N }` faz superfícies de paleta no nível ≥ N pularem o walk de tint e irem direto ao extremo definido por `colorContrastDecompose`; útil em paletas saturadas onde o walk produz texto menos legível que preto/branco puro; WCAG preservado — desde 3.18.0" |
...
| Data | Mudança |
|------|---------|
| 2026-05-15 | Adicionados termos 3.15.1–3.18.0: `theme-engine ai:knowledge`, `theme-engine audit:accessibility`, `global.surfacePolarity`, `ghostNormalTxtOnStrategy`, `colorContrastDecompose`, `overlap (brand-tint)`; atualizado `theme-engine ai:init` e `theme-engine preview` (redesign 3.17.0) |
