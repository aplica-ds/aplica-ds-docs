...
Current version: **3.14.6** (2026-05-08)

- **3.14.0–3.14.6** — Dynamic interaction state overrides + Figma brand bundle splitting: `overrides.interaction` replaces the legacy `overrides.interface.function.*` form and extends it to `feedback` — overrides are scoped by group, item, preset (`solid`/`ghost`), and state (`normal`, `action`, `active`, `focus`, `disabled`), and pass through the full decomposition pipeline (dilution, base adaptation, quadrant mirroring, accessibility text); `figma:generate` now automatically splits oversized brand bundles into semantic chunks (`_brand_product.json`, `_brand_text.json`, `_brand_interface.json`) when the token count exceeds the Figma per-collection limit (default 5,000, overridable via `APLICA_THEME_ENGINE_BRAND_TOKEN_LIMIT`)
- **3.13.5–3.13.8** — Mode-aware feedback decomposition + quadrant mirroring fixes: `options.interaction.decomposition.modeResolution: 'mode'` makes feedback tokens follow light/dark semantics without quadrant polarity inversion — positive and negative surfaces within the same mode produce identical surfaces, text, and borders; bug fixes restore correct quadrant mirroring for dilution-derived colors and for default feedback (`modeResolution: 'quadrant'`) after a regression in 3.13.6
